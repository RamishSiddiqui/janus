...
fn client_info() -> ClientInfo {
    ClientInfo::new(
        ...
        Implementation::new("Janus", env!("CARGO_PKG_VERSION")),
    )
}
...
fn extract_tool_json(result: &CallToolResult) -> Result<serde_json::Value, MythicError> {
    ...
    let text = result
        ...
        .map(|t| t.text.as_str())
        .ok_or_else(|| {
            MythicError::Provider("WanGP tool call returned no parseable content".to_string())
        })?;
    serde_json::from_str(text)
        .map_err(|e| MythicError::Provider(format!("Failed to parse WanGP's response: {}", e)))
}
...
async fn run_wangp_job(
    ...
    if !character_images.is_empty() {
        let schema_result = client
            .call_tool(
                CallToolRequestParams::new("wangp_get_model_schema").with_arguments(
                    serde_json::json!({ "model_type": model_type })
                        .as_object()
                        .cloned()
                        .unwrap_or_default(),
                ),
            ...
            .map_err(|e| {
                MythicError::Provider(format!(
                    "Failed to look up WanGP model schema for {}: {}",
                    model_type, e
                ))
            })?;
    ...
    let generate_result = client
        ...
        .await
        .map_err(|e| {
            MythicError::Provider(format!("WanGP rejected the generation request: {}", e))
        })?;
    ...
    info!(
        "[wangp] Job {} submitted at {} (model_type={})",
        job_id, base_url, model_type
    );
    ...
    loop {
        if cancel_flag.load(Ordering::Relaxed) {
            let _ = client
                .call_tool(
                    CallToolRequestParams::new("wangp_cancel_job").with_arguments(
        ...
        if started.elapsed() > WANGP_MAX_WAIT {
            ...
            return Err(MythicError::Provider(
                "WanGP generation timed out".to_string(),
            ));
        }
        ...
        let job_result = client
            .call_tool(
                CallToolRequestParams::new("wangp_get_job").with_arguments(
                    serde_json::json!({ "job_id": job_id, "event_limit": 5 })
                        .as_object()
                        .cloned()
                        .unwrap_or_default(),
                ),
            ...
            .await
            .map_err(|e| {
                MythicError::Provider(format!("Failed to poll WanGP job {}: {}", job_id, e))
            })?;
        ...
        let Some(result) = job_json.get("result") else {
            continue;
        };
        let success = result
            .get("success")
        ...
        let file_path = result
            ...
            .and_then(|f| f.as_str())
            .ok_or_else(|| {
                MythicError::Provider(
                    "WanGP reported success but returned no generated file".to_string(),
                )
            })?;
...
pub async fn generate_image_via_wangp(
    ...
    let bytes = tokio::fs::read(&file_path).await.map_err(|e| {
        MythicError::Provider(format!(
            "Failed to read WanGP's generated image at {}: {}",
            file_path.display(),
            e
        ))
    })?;
...
pub async fn generate_video_via_wangp(
    ...
    let video_length = (params.duration_seconds * params.fps as f32)
        .round()
        .max(1.0) as u32;
    let mut settings = serde_json::Map::new();
    settings.insert("model_type".to_string(), serde_json::json!(model_type));
    ...
    settings.insert(
        ...
        serde_json::json!(params.duration_seconds),
    );
    if !params.negative_prompt.is_empty() {
        settings.insert(
            "negative_prompt".to_string(),
    ...
    let bytes = tokio::fs::read(&file_path).await.map_err(|e| {
        MythicError::Provider(format!(
            "Failed to read WanGP's generated video at {}: {}",
            file_path.display(),
            e
        ))
    })?;
