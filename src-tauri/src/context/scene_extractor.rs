...
///
/// This is a cheap, fast LLM call (max_tokens=300, temperature=0.1) that runs
/// in the background after streaming completes. If it fails for any reason,
/// the scene state simply remains unchanged.
pub async fn extract_scene_state(
    ...
    let user_prompt = format!(
        ...
        if ai_response.len() > 2000 { &ai_response[..2000] } else { ai_response }
    );
    ...
    debug!("[scene_extractor] Raw output: {}", &raw_output[..raw_output.len().min(200)]);

    // Parse the JSON — be lenient with markdown fences
    ...
    let update: SceneStateUpdate = serde_json::from_str(cleaned)
        .map_err(|e| {
            warn!("[scene_extractor] Failed to parse JSON: {}. Raw: {}", e, &raw_output[..raw_output.len().min(300)]);
