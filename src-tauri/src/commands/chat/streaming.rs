...
pub(crate) struct StreamCompletionCtx {
    ...
    pub(crate) active_gens_cleanup: Arc<tokio::sync::Mutex<std::collections::HashMap<String, crate::GenerationHandle>>>,
...
pub(crate) async fn run_stream_completion(mut ctx: StreamCompletionCtx) {
    let (log_tag, save_err_msg, completed_msg) = match ctx.origin {
        StreamOrigin::Send => ("[send_message]", "Failed to save response", "Chat response completed"),
        StreamOrigin::Retry => ("[retry_failed_message]", "Failed to save retry response", "Retry response completed"),
    };
    ...
    while let Some(chunk) = ctx.rx.recv().await {
        match chunk {
            ...
            StreamChunk::Done(full_text) => {
                ...
                if !is_ordinary_turn {
                    if segments.len() > 1 {
                        info!("[multi-char] Parsed {} character segments{}", segments.len(), suffix);
                        ...
                        let mut created_segments: Vec<serde_json::Value> = Vec::with_capacity(segments.len());
                        ...
                        if let Some((char_id, char_name)) = resolved {
                            info!("[multi-char] Single segment by {}, updating in-place{}", char_name, suffix);
