...
impl SceneRepo {
    ...
    pub async fn create(
        ...
        let query = "CREATE type::thing('scenes', $id) CONTENT {
            conversation_id: type::thing('conversations', $conv_id),
            message_id: IF $msg_id != NONE THEN type::thing('messages', $msg_id) ELSE NONE END,
        ...
        let created: Option<Scene> = result.take(0)?;
    ...
    pub async fn list(db: &Surreal<Db>, conversation_id: &str) -> Result<Vec<Scene>, MythicError> {
        let mut result = db
            .query(
                "SELECT * FROM scenes WHERE conversation_id = type::thing('conversations', $conv_id) ORDER BY created_at DESC",
            )
            .bind(("conv_id", conversation_id.to_string()))
            .await?;
    ...
    pub async fn get_file_path(db: &Surreal<Db>, id: &str) -> Result<Option<String>, MythicError> {
        let mut result = db
            .query("SELECT file_path FROM type::thing('scenes', $id)")
            .bind(("id", id.to_string()))
            .await?;
    ...
    pub async fn delete(db: &Surreal<Db>, id: &str) -> Result<(), MythicError> {
        let _: Option<Scene> = db.delete(("scenes", id)).await?;
        Ok(())
    }
}
